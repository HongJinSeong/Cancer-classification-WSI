...
for kfold_idx, (index_kf_train, index_kf_validation) in enumerate(kf.split(lbls,lbls)):
    ...
    for ep in range(EPOCHs):
        ...
        for imgs, labels, tabular in tqdm(iter(train_loader)):
            ...
            Y_prob, _, A = model(imgs,tabular)

            Y_prob = torch.clamp(Y_prob, min=1e-5, max=1. - 1e-5)
            loss = -1. * (labels * torch.log(Y_prob) + (1. - labels) * torch.log(1. - Y_prob))
            ...
            acc = torch.sum(torch.where(Y_prob >= 0.5, 1, 0)==labels.reshape(-1,1)) / labels.shape[0]
        ...
        with torch.no_grad():
            for imgs, labels, tabular in tqdm(iter(valid_loader)):
                ...
                for v_idx in range(int(imgs.shape[1]/40)):
                    Y_prob, _, A = model(imgs[:, v_idx * 40 : (v_idx+1) * 40, :, :, :],tabular)
