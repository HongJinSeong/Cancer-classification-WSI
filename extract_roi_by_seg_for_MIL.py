...
seg_paths = glob('outputs/segments/output_img/', '*')

dir_path = 'datas/train_imgs_patchs_MIL/'
...
for seg_pth in seg_paths:
    ...
    label = train_csv[train_csv['ID'] == seg_pth.split('/')[-1][:-4]]['N_category'].values[0]
    origin_pth = seg_pth.replace('outputs/segments/output_img', 'datas/train_imgs')
        ...
        if stats[i][2] >= 27 and stats[i][2] >= 27:
            ...
            if w > 100 and h > 100:
                ...
                for idx_box in range(numboxx * numboxy):
                    patch = component_target[new_start_ys[idx_box]:new_start_ys[idx_box] + 27,
                            new_start_xs[idx_box]:new_start_xs[idx_box] + 27]
                    ...
                    if chk_idx[0].shape[0] / (27 * 27) > 0.7:
                        if np.mean(origin_img[new_start_ys[idx_box]:new_start_ys[idx_box] + 27,
                                   new_start_xs[idx_box]:new_start_xs[idx_box] + 27]) < 230:
                            patchs_images.append(origin_img[new_start_ys[idx_box]:new_start_ys[idx_box] + 27,
                                                 new_start_xs[idx_box]:new_start_xs[idx_box] + 27])
            else:
    ...
    save_paths_MIL(patchs_images, save_dir, False)
